<template>
  <div class="c-recommend">
    ...
    <div class="c-recommend-box">
      <div class="recommend-top break">
        <div class="recommend-top__title">推荐歌单</div>
        <!-- <div class="recommend-top__more">
          更多
          <icon type="forword" />
        </div>-->
      </div>

      <el-row :gutter="15" style="margin-top:7.5px;">
        <el-col :span="20" v-for="(item,index) in sheetList" :key="index">
          <sheet-item :item="item" />
        </el-col>
      </el-row>
      <!-- <el-button @click="getRecommend">测试</el-button> -->
    </div>
    <div class="c-recommend-box">
      <div class="recommend-top">
        <div class="recommend-top__title">推荐单曲</div>
      </div>
      <div class="new-song tracks">
        <el-button @click="playAll" size="mini" style="margin:10px 0px 10px 16px;">播放全部</el-button>
        <div class="list">
          <el-table size="small" :data="songList" stripe>
            <el-table-column type="index" width="50"></el-table-column>
            <el-table-column width="60" prop="a_name">
              <template slot-scope="scope">
                <el-image
                  style="width: 40px; height: 40px"
                  :src="scope.row.cover_url ? scope.row.cover_url:require('../../assets/sheetcover1.png')"
                  lazy
                ></el-image>
              </template>
            </el-table-column>
    ...
    <!-- </template>
    <template v-else>
      <div class="not_login">登录后可享受个性化推荐服务</div>
    </template>-->
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    async getRecommendSheet () {
      const result = await api.getRecommendSheet()
      this.sheetList = result.data.sheet_list
    },
    // 测试
    async getRecommend () {
      ...
      let { data, code } = res
      this.songList = data.song_id
      this.sheetList = data.sheet_id
    },
    singerClick (sid) {
    ...
    playAll () {
      this.$store.dispatch('playPlaylist', this.songList)
    }
  }
}
</script>
